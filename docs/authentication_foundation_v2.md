...
### Phase 3 — Production auth provider skeleton (complete)

**Experience Lab frontend:**

- `ProductionAuthProvider` — complete skeleton; structured unavailable/not-implemented responses
- `fetchAuthHttpClient.ts` — production `fetch()` client with `credentials: 'include'`
- `authHttpRequestBuilder.ts` — centralized request builder (headers, timeout, retry hooks)
- `authHttpResponseParser.ts` — HTTP → Session / AuthenticationResult / AuthError
- `authProductionConfig.ts` — API base URL, timeout, cookie expectations, retry policy
- `authHttpLogging.ts` — safe logging boundaries documented
- Environment: `development` / `preview` → MockAuthProvider; `production` + `VITE_AUTH_BACKEND_ENABLED=true` → ProductionAuthProvider
- Graceful failure: professional message on login when production auth unavailable; never throws to UI
...
### Phase 6 — User & role management UI
...
### Phase 7 — Organization permissions
...
### Phase 8 — Production preview hardening
...
## 18. Phase 3 Implementation — Production Provider Skeleton

### 18.1 Architecture

```
AuthService → resolveAuthProviderKind()
  development / preview → MockAuthProvider → MockAuthHttpClient
  production + VITE_AUTH_BACKEND_ENABLED=true → ProductionAuthProvider → FetchAuthHttpClient
                                                                              ↓
                                                                   executeAuthRequest()
                                                                              ↓
...
### 18.2 Environment selection

| Runtime (`VITE_AUTH_RUNTIME`) | Backend (`VITE_AUTH_BACKEND_ENABLED`) | Active provider |
|-------------------------------|---------------------------------------|-----------------|
| `development` (default local) | any | **MockAuthProvider** |
| `preview` (default PROD build) | any | **MockAuthProvider** |
| `production` | `false` / unset | **MockAuthProvider** (fallback) |
| `production` | `true` | **ProductionAuthProvider** |

### 18.3 Request builder

Centralized in `executeAuthRequest()`: base URL, `Accept` / `Content-Type`, `credentials: 'include'`, timeout, reserved correlation/telemetry headers.

### 18.4 Response parser

HTTP responses parsed to `Session`, `SignInResult`, `MfaVerifyResult`, and `AuthError` — UI never reads raw `Response` objects.

### 18.5 Logging policy

**Safe:** request IDs, timestamps, endpoints, method, duration, status.  
**Never:** passwords, MFA codes, cookies, session IDs, secrets, credential bodies.

### 18.6 Production readiness checklist

| Item | Status |
|------|--------|
| ProductionAuthProvider skeleton | ✓ |
| FetchAuthHttpClient | ✓ |
